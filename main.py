# -*- coding: utf-8 -*-
"""
简衣服饰旗舰店 AI 智能客服 Demo
- FastAPI 同时提供前端页面与 /chat 接口（单服务部署，最终只有一个链接）
- 大模型：DeepSeek（deepseek-chat）
- 多轮对话：前端携带历史消息，后端组装后发给大模型
- 密钥安全：DEEPSEEK_API_KEY 只从环境变量 / .env 读取，不写死在代码里
"""
import os
...
app = FastAPI(title="简衣服饰 AI 客服 Demo")

# 同源部署其实不需要跨域，保留是为了本地 file:// 调试等场景
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
MAX_HISTORY_TURNS = 10  # 只携带最近10轮历史，控制 token 消耗
...
def chat(body: ChatBody):
    if not DEEPSEEK_API_KEY:
        return {"reply": "后端尚未配置 DEEPSEEK_API_KEY，请在 .env 文件或平台环境变量中配置后重启服务。"}

    # 组装消息：系统提示词 + 最近多轮历史 + 本轮用户消息
    messages = [{"role": "system", "content": SYSTEM_PROMPT}]
    for item in body.history[-MAX_HISTORY_TURNS:]:
        role = item.get("role")
    ...
    try:
        ...
        return {"reply": f"大模型接口返回错误（HTTP {resp.status_code}）：{resp.text[:300]}"}
    except Exception as e:  # noqa: BLE001
        return {"reply": f"后端服务异常：{e}"}
